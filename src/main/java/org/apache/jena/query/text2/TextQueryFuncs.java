...
public class TextQueryFuncs {
    ...
    public static Entity entity(EntityDefinition defn, Node g, Node s) {
    	String sx = subjectToString(s);
        String gx = TextQueryFuncs.graphNodeToString(g) ;
        Entity entity = new Entity(sx, gx) ;
        String graphField = defn.getGraphField() ;
        if ( defn.getGraphField() != null )
    ...
    public static boolean addPropertyToEntity(EntityDefinition defn, Entity entity, Node p, Node o) {
    	String field = defn.getField(p) ;
        if ( field == null )
            return false ;
        if ( !o.isLiteral() ) {
