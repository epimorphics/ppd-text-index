...
public class TextDatasetFactory
{
    ...
    {
        return create(base, textIndex, null) ;
    }
    ...
    public static Dataset create(Dataset base, TextIndex textIndex, TextDocProducer docProducer)
    {
        ...
        dsg = create(dsg, textIndex, docProducer) ;
    ...
    {
    	return create(dsg, textIndex, null);
    }

    /** Create a text-indexed dataset */ 
    public static DatasetGraph create(DatasetGraph dsg, TextIndex textIndex, TextDocProducer docProducer)
    {
    	if (docProducer == null) {
    		docProducer = new TextDocProducerTriples() ;
        }
    	docProducer.setDefn(textIndex.getDocDef());
    	docProducer.setTextIndex(textIndex);
    	docProducer.setDatasetGraph(dsg);
    	
        DatasetGraph dsgt = new DatasetGraphText(dsg, textIndex, docProducer) ;
        // Also set on dsg
        Context c = dsgt.getContext() ;
        
        dsgt.getContext().set(TextQuery.textIndex, textIndex) ;
        dsgt.getContext().set(TextQuery.docProducer, docProducer ) ;
        return dsgt ;
    ...
    {
    	return createLucene(base, directory, entMap, null) ;
    }
    ...
    /** Create a text-indexed dataset, using Lucene */ 
    public static DatasetGraph createLucene(DatasetGraph base, Directory directory, EntityDefinition entMap, TextDocProducer docProducer)
    {
        TextIndex index = createLuceneIndex(directory, entMap) ;
        return create(base, index, docProducer) ; 
    }
    ...
    {
        return createSolrIndex(base, server, entMap, null) ; 
    }

    /** Create a text-indexed dataset, using Solr */ 
    public static Dataset createSolrIndex(Dataset base, SolrServer server, EntityDefinition entMap, TextDocProducer docProducer)
    {
    ...
    /** Create a text-indexed dataset, using Solr */ 
    public static DatasetGraph createSolrIndex(DatasetGraph base, SolrServer server, EntityDefinition entMap, TextDocProducer docProducer)
    {
        TextIndex index = createSolrIndex(server, entMap) ;
        return create(base, index, docProducer) ; 
    }
